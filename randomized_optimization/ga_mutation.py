...
def train(oa, network, oaName, train_set, test_set, measure, mutate_each_gen):
    ...
    fname = 'out/ga/mutation-%d.csv' % (mutate_each_gen)
...
def main():
    ...
    for mutate_each_gen in [10, 25, 50]:
        ...
        oa = StandardGeneticAlgorithm(GA_POPULATION_SIZE, GA_MATE_EACH_GEN, mutate_each_gen, nnop)

        start = time.time()
        correct = 0
        incorrect = 0

        train(oa, network, "GA", train_set, test_set, measure, mutate_each_gen)
        end = time.time()
        ...
        _results = ""
        _results += "\n[GA] mutation=%0.02f" % (mutate_each_gen)
        ...
        with open('out/ga/mutation-%d.log' % (mutate_each_gen), 'w') as f:
            f.write(_results)
